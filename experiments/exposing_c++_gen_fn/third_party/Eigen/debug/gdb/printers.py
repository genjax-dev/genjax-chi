...
class EigenSparseMatrixPrinter:
    ...
    class _Iterator(_MatrixEntryIterator):
        ...
        def __next__(self):
            row, col = super(
                EigenSparseMatrixPrinter._Iterator, self
            ).__next__()
    ...
    def children(self):
        if self.data:
            return self._Iterator(
                self.rows(), self.cols(), self.val, self.rowMajor
            )
    ...
    def rows(self):
        return (
            self.val["m_outerSize"]
            if self.rowMajor
            else self.val["m_innerSize"]
        )

    def cols(self):
        return (
            self.val["m_innerSize"]
            if self.rowMajor
...
def build_eigen_dictionary():
    ...
    pretty_printers_dict[
        re.compile("^Eigen::Block<.*>$")
    ] = lambda val: EigenMatrixPrinter(
        "Matrix", cast_eigen_block_to_matrix(val)
    )
    pretty_printers_dict[
        re.compile("^Eigen::VectorBlock<.*>$")
    ] = lambda val: EigenMatrixPrinter(
        "Matrix", cast_eigen_block_to_matrix(val)
    )
    pretty_printers_dict[
