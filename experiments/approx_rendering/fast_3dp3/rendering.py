...
def render_planes(pose, shape, h, w, fx_fy, cx_cy):
    ...
    pixel_coords_dir = jnp.concatenate(
        [(pixel_coords - cx_cy) / fx_fy, jnp.ones((h, w, 1))], axis=-1
    )

    denoms = jnp.einsum("ijk,ak->ija", pixel_coords_dir, plane_poses[:, :3, 2])
    numerators = jnp.einsum(
        "...k,...k", plane_poses[:, :3, 3], plane_poses[:, :3, 2]
    )
    d = numerators / denoms
    points_temp = jnp.einsum(
        "...ij,...kj", d[:, :, :, None], pixel_coords_dir[:, :, :, None]
    )
    ...
    points_in_plane_frame = jnp.einsum(
        "...ij,ab...j->ab...i", inv_plane_poses, points
    )

    valid = jnp.all(
        jnp.abs(points_in_plane_frame[:, :, :, :2]) < shape_dimensions, axis=-1
    )  # (H,W,N)
    ...
    points_final = (
        points[
            jnp.arange(points.shape[0])[:, None],
            jnp.arange(points.shape[1])[None, :],
            idxs,
        ]
