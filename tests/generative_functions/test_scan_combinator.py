...
class TestScanMisc:
    ...
    def test_scan_of_repeat(self, key):
        @genjax.gen
        def kernel(x, _):
        ...
        scanned = kernel.scan(n=10)

        key, k1, k2, k3, k4, k5 = jax.random.split(key, 4)
        tr = scanned.simulate(k1, (jnp.zeros(4), None))
        assert (
            tr.project(k2, S[..., "d1"]) + tr.project(k3, S[..., "d2"])
            == tr.get_score()
        )
        assert (
            tr.project(k4, C[jnp.array([0, 1, 2, 3, 4]), ...])
            + tr.project(k5, C[jnp.array([5, 6, 7, 8, 9]), ...])
            == tr.get_score()
        )
