...
@Pytree.dataclass
class Checkerboard(EditRequest):
    edit_request: EditRequest
    ...
    def edit(
        ...
        argdiffs: Argdiffs,
    ) -> tuple[ScanTrace[Carry, Y], Weight, Retdiff[Any], "EditRequest"]:
        assert Diff.static_check_no_change(argdiffs)
        ...
        num_indices = len(checkerboard_indices)

        def parallel_edit(key, idx):
            request = IndexRequest(idx, self.edit_request)
            new_tr, w, retdiff, bwd_index_request = request.edit(key, tr, argdiffs)
            assert isinstance(bwd_index_request, IndexRequest)
            return new_tr, w, retdiff, bwd_index_request.request
        ...
        def indexer(v):
            ...
            ret = jax.vmap(
                lambda idx, v: jax.lax.dynamic_slice_in_dim(v, idx, 2, axis=0)
            )(jnp.arange(num_indices), v)
            return ret.flatten()

        final_inner_trace = jtu.tree_map(
            indexer,
            parallel_trace.inner,
        )
        retdiff = jtu.tree_map(lambda v: v[-1], retdiffs)
        retval = Diff.tree_primal(retdiff)
        scores = jax.vmap(lambda tr: tr.get_score())(final_inner_trace)
