...
class InitialStylePrimitive(jc.Primitive):
    """A primitive which is expected to be eliminated by an initial
    style interpreter"""
    ...
    def __init__(self, name):
        ...
        def _abstract(*flat_avals, **params):
            abs_rule = params["abs_rule"]
            return abs_rule(*flat_avals, **params)
...
def initial_style_bind(prim, **params):
    ...
    def bind(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            def abs_rule(*flat_avals, **params):
                return pe.abstract_eval_fun(impl, *flat_avals, **params)

            outs = prim.bind(
                *it.chain(jaxpr.literals, flat_args),
                impl=impl,
                abs_rule=params.get("abs_rule", abs_rule),
                in_tree=in_tree,
                out_tree=out_tree,
