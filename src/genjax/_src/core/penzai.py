# This file was copied over from
# https://github.com/google-deepmind/penzai/blob/d07f5cb8a30bc236c34df6e8755b0b5217c0b110/penzai/core/struct.py
# with slight modifications to get the typechecker to pass.
#
#
...
import jax
from beartype.typing import Any, Callable, Hashable, Sequence, TypeVar
from treescope import dataclass_util, formatting_util, renderers, rendering_parts
    ...
    *mutable subclass* of the class (stored as `cls._MutableInitProxy`), using the
    ...
    if init and original_init is None and not overwrite_parent_init:
        ...
        if hasattr(cls, PYTREE_DATACLASS_INFO_ATTR):
            ...
            generated_init = getattr(cls, PYTREE_DATACLASS_INFO_ATTR)["generated_init"]
    ...
    if has_implicitly_inherited_fields:
        ...
        explicit_field_names = [
            name for name in explicit_annotation_names if name in actual_field_name_set
        ]
    ...
    if not (hasattr(cls, "tree_flatten_with_keys") and hasattr(cls, "tree_unflatten")):
...
class Struct(metaclass=AbstractStructMetaclass):
    ...
    * Some equinox-specific features are not supported. Specifically, bound
    ...
    def from_attributes(cls: type[T], **field_values) -> T:
        ...
        expected_fields = dataclasses.fields(cls)  # pyright: ignore
    ...
    def tree_flatten_with_keys(
        ...
        static_fields = {}
        # `self` is guaranteed to be a dataclass by AbstractStructMetaclass
        for field_ in dataclasses.fields(self):  # pyright: ignore
...
def struct_attr_style_fn_for_fields(
    ...
    def attr_style_fn(field_name):
        if is_pytree_node_field(fields_by_name[field_name]):
