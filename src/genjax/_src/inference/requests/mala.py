...
class MALA(EditRequest):
    """
    Apply a Metropolis-adjusted Langevin ascent (MALA) update that proposes new values for the selected addresses,
    returning the new trace, and a weight which is equal to the alpha accept-reject ratio computation for MALA.
    ...
    def edit(
        ...
        return (
            ...
            alpha,
            retdiff,
            MALA(self.selection, self.eps, self.L),
        )
...
def SafeMALA(
    ...
    return MALA(selection, eps, L).map(retdiff_assertion)
