...
# %%
comparison_plot = Plot.new()


def reset_comparison_plot():
    global key
    key, subkey = jax.random.split(key)
    _, jax_gradients = compute_jax_vals(subkey, INITIAL_VAL)
    _, adev_gradients = compute_adev_vals(subkey, INITIAL_VAL)
    plot = (
        Plot.line(
            jax_gradients,
            {"x": Plot.js("(_, i) => i"), "y": "1"},
        ...
        "div.inline-block",
        {"class": button_classes, "onClick": lambda _: reset_comparison_plot()},
        "Refresh",
    ]
    comparison_plot.reset(plot)


reset_comparison_plot()
comparison_plot
